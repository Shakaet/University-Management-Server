...
import { assignFacultyInfoDB, assignFacultyWithCourseIntoDB, creeateCoursesIntoDb, deletecoursesFromDb, getAllCoursesFromDb, getFacultyWithCoursefromDB, getSingleCoursesFromDb, removedFacultyWithCourseFromDB, updateCoursesIntoDb } from './course.service'
...
export const getFacultiesWithCourse: RequestHandler = async (req, res, next) => {
  try {
    const id = req.params.courseId

    

    const result = await getFacultyWithCoursefromDB(id)

    res.status(200).json({
      status: true,
      messsage: 'CourseFaculty retrive successfully',
      data: result,
    })
  } catch (err: any) {
    //  res.status(500).json({
