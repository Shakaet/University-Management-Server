import { iSemesterNameEqualSemesterCode, TacademicSemester } from "./academicSem.interface"
...
export let createAcademicSemToDb=async(payload:TacademicSemester)=>{
    ...
    if(iSemesterNameEqualSemesterCode[payload.name]!==payload.code){
        ...
        throw new Error(`${payload.name} is not valid Semester Name for code ${payload.code} `)
    }
